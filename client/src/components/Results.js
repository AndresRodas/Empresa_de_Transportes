...
const Results = () => {
    ...
    const [currentCapacidad, setCurrentCapacidad] = useState(''); 
    const [currentUbicacion, setCurrentUbicacion] = useState('');
    const [currentDepreciacion, setCurrentDepreciacion] = useState(''); 
    const [currentTipoCarga, setCurrentTipoCarga] = useState(''); 
    ...
    const updateCar = () => {
        if(currentPlaca !== '' && currentMarca !== '' && currentModelo !== ''){
            const newVehicle = {
                id: currentId,
                placa: currentPlaca,
                marca: currentMarca,
    ...
    return (
        <div>
            ...
            currentPageReportTemplate="Showing {first} to {last} of {totalRecords} entries">
                ...
                <Column field="capacidad" header="Capacidad" sortable filter filterPlaceholder="Search by Serie"></Column>
                <Column field="ubicacion" header="Ubicacion" sortable filter filterPlaceholder="Search by Color"></Column>
                <Column field="depreciacion" header="Depreciacion" sortable filter filterPlaceholder="Search by Serie"></Column>
                <Column field="carga" header="Tipo de Carga" sortable filter filterPlaceholder="Search by Color"></Column>
            ...
            <Sidebar visible={upVisible} position="left" className="p-sidebar-sm" onHide={() => { setUpVisible(false); setCurrentPlaca('');}}>
                ...
                <div className="flex flex-column card-container mx-8 my-4">
                    ...
                    <div className="my-2 flex align-items-center justify-content-center">
                        <InputText placeholder='Capacidad' value={currentCapacidad} onChange={(e) => setCurrentCapacidad(e.target.value)} />
                    </div>
                    <div className="my-2 flex align-items-center justify-content-center">
                        <InputText placeholder='Ubicación' value={currentUbicacion} onChange={(e) => setCurrentUbicacion(e.target.value)} />
                    </div>
                    <div className="my-2 flex align-items-center justify-content-center">
                        <InputText placeholder='Depreciacion' value={currentDepreciacion} onChange={(e) => setCurrentDepreciacion(e.target.value)} />
                    </div>
                    <div className="my-2 flex align-items-center justify-content-center">
                        <InputText placeholder='Tipo Carga' value={currentTipoCarga} onChange={(e) => setCurrentTipoCarga(e.target.value)} />
                    </div>
